...
public class BlockCFSlabA extends  BlockHalfSlab {

	/** The type of tree this slab came from. */
    public static final String[] type = new String[] {"blackCF", "redCF", "greenCF", "brownCF", "blueCF", "purpleCF", "cyanCF", "lightGrayCF"};

    public BlockCFSlabA(int par1, boolean par2)
    {
    ...
    {
    	if (par2 < 8) {
    		return par2 + 16; //offset nel texture file + offset per lo slabB
    	}
    	else {
    		return par2 + 8; //per gli slab > 8, va sottratto 8 per ricominciare le texture da 0
    	}
    }
    ...
    public int idDropped(int par1, Random par2Random, int par3)
    {
        //return Block.woodSingleSlab.blockID;
   		return Blocks.cfSingleSlabA.blockID;
    }
    ...
    {
        return new ItemStack(Blocks.cfSingleSlabA.blockID, 2, par1 & type.length);
    }
    ...
    {
        if (par1 != Blocks.cfSlabA.blockID)
        {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
    	try {
	    	...
	    	if (is.itemID >= PAINTER_ID && is.itemID < (PAINTER_ID + 16)) {
	    		...
	    		//gestione dello split a :8 in 2 Dato che siamo nel CFSlabA allora sopra meta=7 va aggiunto 2 al blockid e sottratto 8 al meta per passare al B.
	    		...
	    		//System.out.println("A inputID: " + is.itemID + " rawMeta: " + rawMeta + " blockID: " + blockID + " meta: " + meta);
